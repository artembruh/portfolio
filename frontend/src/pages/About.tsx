...
export default function About() {
  return (
    <div>
      ...
      <h1 className="text-3xl mb-1.5" style={{ textShadow: '0 0 10px #ffd52c66' }}>
        Artem Bratchenko
      </h1>
      <p className="text-lg opacity-70 leading-7 mb-0">
        Senior Backend Engineer
      </p>
      <p className="text-base opacity-50 leading-7 max-w-2xl mt-2">
      ...
      {education.map((edu) => (
        <TerminalCard key={edu.year}>
          ...
          <div className="text-base opacity-50">{edu.institution} · {edu.year}</div>
        </TerminalCard>
      ))}
      ...
      {stack.map((category) => (
        <div key={category.label} className="mb-2.5">
          <div className="text-sm opacity-40 uppercase tracking-wider mb-1.5">{category.label}</div>
          <div>
      ...
      {/* Mobile social links */}
      <div className="md:hidden mt-8 pt-4 border-t border-[var(--pip-primary)]/15 text-sm opacity-30 space-y-1">
